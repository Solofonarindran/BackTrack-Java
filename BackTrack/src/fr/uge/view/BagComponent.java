...
//	private final int girdStartY;
  ...
  private static final Color HIGHLIGHT_INSUFFICIENT = new Color(255, 100, 100, 100);
	...
	public static BagComponent create(int xResolution, int yResolution, BackPack bag) {
		var x =(int) (xResolution * 0.08);
		var y = (int) (yResolution * 0.30);
  ...
  public void draw(Graphics2D g, Component component) {
    ...
    return switch (item) {
        case Weapon _ -> new Color(200, 80, 80);      // Rouge
        case Armor _ -> new Color(80, 80, 200);       // Bleu
        case Consumable _ -> new Color(80, 200, 80);  // Vert
        case Magic _ -> new Color(200, 80, 200);      // Violet
        case Gold _ -> new Color(255, 200, 50);       // Or
        case Curse _ -> new Color(100, 50, 100);      // Violet foncé
        default -> new Color(150, 150, 150);          // Gris par défaut
    };
	}
    ...
    var tooltipX = bounds[0] + bounds[2] + 10;
...
//Dans BagComponent ou CombatController

	public void handleRotation(Item item) {
	   if (item == null) return;
	   ...
	   var success = bag.rotateEquipment(item);
	   if (success) {
	       System.out.println("Item tourné !");
	   } else {
	       System.out.println("Impossible de tourner ici !");
	   }
	}
